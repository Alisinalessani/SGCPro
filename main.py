...
# - خودکار: کشف Plate/MJD/Fiber با subclass معتبر
# - بهبودها: برآورد z، حذف پیوستار، تشخیص قله/دِپ، نسبت‌های شبه-BPT، قوانین غنی
# - خروجی: SGC code + عناصر + z + نسبت‌ها + ارزیابی کمی

# اگر در نوت‌بوک اجرا می‌کنی، این خط را باز بگذار:
# !pip install -q astroquery astropy scipy pandas tqdm

from astroquery.sdss import SDSS
import numpy as np
import scipy.signal as sig
from scipy.ndimage import median_filter as nd_median_filter  # <— افزودنی: جایگزین medfilt
import pandas as pd
...
PEAK_DIST = 5
TOL_Z_GRID = 0.0005
Z_MIN, Z_MAX = -0.005, 0.40   # allow small negative for stars with slight calibration offsets
LINE_TOL = 4.5                # tolerance (Angstrom) in rest-frame to accept a match
...
def rolling_median(x, win):
    """
    نسخه‌ی مقاوم: به جای scipy.signal.medfilt از scipy.ndimage.median_filter استفاده می‌کنیم
    تا مشکل ValueError: dtype=>f4 is not supported by medfilt رفع شود.
    """
    if win % 2 == 0:
        win += 1
    if win < 5 or win > len(x):
        return x
    # nd_median_filter روی همه‌ی dtypeهای عددی کار می‌کند.
    x = np.asarray(x)
    try:
        return nd_median_filter(x.astype(np.float64, copy=False), size=int(win), mode='nearest')
    except Exception:
        # در صورت بروز استثناء نادر، برای اینکه رفتار کد تغییر نکند همان x را برمی‌گردانیم.
        return x

def preprocess(lam, flux):
...
def estimate_z(lam, y):
    # peaks for emission alignment
    ...
    # build quick index: observed features
    obs_lines = np.concatenate([peaks_wave, trough_wave])
    obs_prom  = np.concatenate([peaks_prom, trough_prom])

    # score z by aligning emission lines (more reliable for galaxies)
...
def measure_line_strengths(lam, y, z_est):
    # shift to rest frame
    lam_rest = lam / (1.0 + z_est)
    (p_idx, p_props), (t_idx, t_props) = find_peaks_troughs(y)
    peaks = { "wave": lam_rest[p_idx], "prom": p_props.get("prominences", np.ones_like(p_idx)), "width": p_props.get("widths", np.ones_like(p_idx)) }
    troughs = { "wave": lam_rest[t_idx], "prom": t_props.get("prominences", np.ones_like(t_idx)), "width": t_props.get("widths", np.ones_like(t_idx)) }

    strengths = {}
    def nearest_feature(w0, is_emission):
        waves = peaks["wave"] if is_emission else troughs["wave"]
        prom  = peaks["prom"] if is_emission else troughs["prom"]
        if waves.size == 0:
            return None
        diff = np.abs(waves - w0)
        j = np.argmin(diff)
        if diff[j] <= LINE_TOL:
            return float(prom[j]), float(diff[j])
        return None

    # Emission lines
    for (name, w0, _w) in EMISSION_PRIOR:
        hit = nearest_feature(w0, is_emission=True)
        strengths[name] = hit[0] if hit is not None else 0.0
    # Absorption lines (depth)
    for (name, w0, _w) in ABSORPTION_PRIOR:
        hit = nearest_feature(w0, is_emission=False)
        strengths[name] = hit[0] if hit is not None else 0.0

    return strengths

def classify_rules(stren):
    # line ratios (approx from peak/trough prominences after continuum-removal)
    eps = 1e-8
    ...
    # --- Galaxy/Nebula branch ---
    if em_count >= 2 and ha + hb + o3 > 0.05:
        # AGN-like: high [OIII]/Hb or high [NII]/Ha or high [SII]/Ha
        if (r_o3_hb >= 3.0) or (r_n2_ha >= 0.6) or (r_s2_ha >= 0.5):
            return "AGN Galaxy", {
                "r_OIII_Hb": r_o3_hb, "r_NII_Ha": r_n2_ha, "r_SII_Ha": r_s2_ha,
                "reason": "High line ratios (AGN-like)."
            }
        # HII / Star-forming: Balmer + [OIII] present, low NII/SII
        if (ha > 0 and hb > 0) and (o3 > 0) and (r_n2_ha < 0.6) and (r_s2_ha < 0.5):
            return "Star-forming Galaxy", {
                "r_OIII_Hb": r_o3_hb, "r_NII_Ha": r_n2_ha, "r_SII_Ha": r_s2_ha,
                "reason": "Balmer + [OIII] with low [NII]/[SII]."
            }
        # Nebula-like: strong [OIII]/[SII], weak absorption signature
        if (o3 > 0.6 or s2 > 0.6) and (abs_count <= 1):
            return "Nebula (HII or PN)", {
                "r_OIII_Hb": r_o3_hb, "r_NII_Ha": r_n2_ha, "r_SII_Ha": r_s2_ha,
                "reason": "Strong forbidden lines, weak stellar absorption."
            }
    ...
    if abs_count >= 2 and em_count <= 1:
        if (ca > 0.5 and na > 0.4 and mgb > 0.4):
            return "K-type Star", {
                "CA": ca, "NA": na, "MGB": mgb, "reason": "Strong Ca II/Na D/Mg b absorption."
            }
        if (ca > 0.3 and (cn > 0.25 or mgb > 0.25) and na < 0.6):
            return "G-type Star", {
                "CA": ca, "CN": cn, "MGB": mgb, "NA": na, "reason": "Ca II + CN/Mg b, moderate Na D."
            }
...
# ============ SGC code (kept for compatibility) ============
def encode_sgc_from_rest(lam_rest, y_rest, tau1, tau2, pad_length=PAD_LEN):
    # use peaks (emission-like) in rest space for spacing code
    try:
...
# =========================
def map_sdss_to_label(sdss_class, sdss_subclass):
...
labeled = discover_labeled_samples()
if len(labeled) == 0:
...
# =========================
# Estimate global taus (from emission-peak spacing in rest frame using estimated z)
# =========================
...
# =========================
# Evaluation (no sklearn)
# =========================
if len(eval_rows) == 0:
    print("\nNo evaluable samples (no mapped ground-truth). Consider increasing MAX_PLATES/FIBERS_PER_PLATE.")
else:
